window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      if (window.__dshTokenUsageInstalled) return;
...
var PROVIDER_ALIASES = {
      ...
      function injectStyle() {
        ...
        var css = [
          ...
          '#dsh-tu-modal .dsh-tu-panel{position:relative;z-index:1;width:920px;max-width:calc(100vw - 40px);max-height:82vh;overflow:auto;background:var(--dsw-alias-bg-layer-2, #ffffff);color:var(--dsw-alias-label-primary, #1f2937);border:1px solid var(--dsw-alias-line, rgba(0,0,0,.08));border-radius:16px;padding:18px 20px;box-shadow:0 18px 60px rgba(0,0,0,.22);}',
          '#dsh-tu-modal .dsh-tu-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:10px;}',
          '#dsh-tu-modal .dsh-tu-head h3{margin:0;font-size:16px;}',
      ...
      function renderModal() {
        ...
        var sourceLabel = state.pricingSource === 'dsh-assets' ? 'DSH 内置价格库'
        ...
        html += '<table><thead><tr><th>模型</th><th>Provider</th><th>会话数</th><th>输入(未缓存)</th><th>缓存读取</th><th>输出</th><th>估算费用</th></tr></thead><tbody>';
        d.models.forEach缓存写入</th><th>(function (m) {
          var p = resolvePrice(m.provider, m.model);
          var unpriced = !p || (!p.input && !p.output && !p.cacheRead && !p.cacheWrite);
          html += '<tr><td>' + escapeHtml(m.model) + '</td><td class="dsh-tu-muted">' + escapeHtml(m.provider || '—') + '</td><td>' + m.sessions + '</td><td>' + fmtInt(m.input) + '</td><td>' + fmtInt(m.cacheRead) + '</td><td>' + fmtInt(m.output) + '</td><td>' + fmtInt(m.cacheWrite) + '</td><td class="' + (unpriced ? 'dsh-tu-unpriced' : '') + '">' + (unpriced ? '未定价/免费' : money(m.cost)) + '</td></tr>';
        });
        html += '</tbody></table>';

        html += '<h3 style="font-size:14px;margin:14px 0 6px;">按会话统计</h3>';
        html += '<table><thead><tr><th>会话 / 标题</th><th>工作区</th><th>模型</th><th>输入(未缓存)</th><th>缓存读取</th><th>输出</th><th>估算费用</th></tr></thead><tbody>';
        d.rows.forEach缓存写入</th><th>(function (r) {
          var modelNames = (r.models || []).map(function (m) { return m.model; }).join(', ') || '—';
          html += '<tr><td>' + escapeHtml(r.title) + '</td><td class="dsh-tu-muted">' + escapeHtml(r.cwd) + '</td><td>' + escapeHtml(modelNames) + '</td><td>' + fmtInt(r.totals.input) + '</td><td>' + fmtInt(r.totals.cacheRead) + '</td><td>' + fmtInt(r.totals.output) + '</td><td>' + fmtInt(r.totals.cacheWrite) + '</td><td>' + money(r.totals.cost) + '</td></tr>';
        });
        html += '</tbody></table>';
        panel.innerHTML = html;
      }
